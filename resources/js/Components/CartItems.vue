<template>
    <div class="flex justify-between border-t border-b border-black py-2" v-if="count > 0">
        ...
        <div class="flex justify-between 2-1/2">
    ...
    <div>
        <div class="flex justify-between border-b border-black py-2" v-for="(item, index) in items" :key="index">
            ...
            <!-- options.slug defined in CartController store() -->
                <Link :href="route('shop.show', item.options.slug)" class="flex flex-1">
                    <img :src="'/storage/images/defaults/'+item.options.image" :alt="item.name" class="object-cover">
                </Link>
                <div class="flex flex-1 flex-col justify-between">
                    ...
                    <div class="flex flex-col mt-4">
                        <form @submit.prevent="remove(item.rowId)"><!-- 'remove' prop -->
                            <button type="submit" class="hover:text-yellow-500">
                                Remove
                            </button>
                        </form>
                        <!-- data source: inspect->vue->props/cartItems/object/options/rowId -->
            ...
            <div class="flex justify-between w-1/2">
                <div class="flex-1 text-center">
                    <!-- use v-model https://www.youtube.com/watch?v=W6OfExrkiBU -->
                    <select class="border bg-white rounded outline-none py-0" tabindex="1" v-model="item.qty" @change="update(item.rowId, item.qty)">
                        <option :value="qty" :selected="item.qty" v-for="(qty, index) in item.options.totalQty" :key="index">
                            {{ qty }}
                        </option>
                    </select>
                </div>
...
export default defineComponent({
    props: ['count', 'items', 'later', 'remove', 'actionText', 'update'],
    components: {
        Link,
    },
